...
# data reorganization
HSC_morph_hr$hr_c <- factor(HSC_morph_hr$hr_c, levels = c('Pre', 'Post'))
# 3 levels of hr were taken; I am only concerned with 2 for this project, 
# pre and post (exclude post 2)
```

```{r temperature and amebocyte density boxplot}
...
```{r amebocyte density linear model}
ameb_d_temp_age_mod <- lm(ameb_d ~ temp + age + temp : age, 
                          data = HSC_age_dens)
summary(ameb_d_temp_age_mod)
library(car)
Anova(ameb_d_temp_age_mod, type = 3)
# statistical summary of results
# no signifcance found in any variables

par(mfrow = c(2,2))
plot(ameb_d_temp_age_mod)
# tests assumptions of models
```

```{r power analysis temperature and ameb density}
library(dplyr)
...
```{r power test function to look at different powers}
# function/plot to analyze different powers
sig_df <- c(0.5, 0.75, 0.9) # different powers that i would like to analyze
# goal: make graph that evaluates number of samples needed at different power
# levels
...
```{r age and amebocyte density power test}
library(dplyr)
...
pow_age_dens_n <- power.t.test(n= , delta = 500000, sd = sigma_ameb_d, 
                               power = 0.5,sig.level =  0.05)
pow_age_dens_n
# gives n = 13... number of hscs required for each group

pow_age_dens_p <- power.t.test(n = 6, delta = 500000, 
                                 sd = sigma_ameb_d, power = ,
                                 sig.level =  0.05)
pow_age_dens_p
# gives 0.24 power...not very good
```


```{r temperature and amebocyte morphology box plot}
...
# code to include legend; excluded in this case because it blocks graph
...
```{r age and temperature morphology sig testing}
age_temp_mod <- lm(morph_p ~ morph_c + temp + age + temp:age, 
                   data = HSC_morph_hr)
...
```{r power test for morph and temp}
...
boxplot(hr ~ hr_c + age, data = HSC_morph_hr,
        ...
        col = c('springgreen4', 'plum', 'springgreen4', 
                'plum')
        )
```
...
```{r hr age and temperature anova testing}
hr_mod <- lm(hr ~ hr_c + age + temp + age : temp, data = HSC_morph_hr)
summary(hr_mod)
car::Anova(hr_mod, type = 3)
```